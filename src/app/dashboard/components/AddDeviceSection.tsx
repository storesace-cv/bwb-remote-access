...
export function AddDeviceSection({
  ...
  // Se não tiver nenhuma das permissões de adicionar dispositivo, não mostrar a secção
  const canScanQr = userPermissions?.can_scan_qr ?? false;
  const canProvisionWithoutQr = userPermissions?.can_provision_without_qr ?? false;
  
  if (!canScanQr && !canProvisionWithoutQr) {
    return null;
  }
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {/* Provisioning sem QR - requer can_provision_without_qr */}
        {canProvisionWithoutQr && (
          <Link
            href="/provisioning"
      ...
      {/* APK Download QR Codes - só mostra se tiver permissão de scan QR */}
      {canScanQr && (
